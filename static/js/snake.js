...
window.onload = function() {
    ...
    // Create objects
    var snake = new Snake();
    var level = new Level(20, 15, 39, 39);
    ...
    function init() {
      // Load images
      images = loadImages(["static/images/snake-graphics.png"]);
      tileimage = images[0];
      // Add mouse events
    ...
    function updateGame(dt) {
      input.on("keyup", function(e) {
        if (e.keyCode === 13) {   
          if (input.val() === words[up.val()]) {
            if (snake.direction != 2) {
              snake.direction = 0;
            }
          } else if (input.val() === words[down.val()]) {
            if (snake.direction != 0) {
              snake.direction = 2;
            }
          } else if (input.val() === words[left.val()]) {
            if (snake.direction != 1) {
              snake.direction = 3;
            }
          } else if (input.val() === words[right.val()]) {
